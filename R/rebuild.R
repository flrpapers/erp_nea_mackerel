...
rebuild<-function(object,n=51,nyear=50,burnin=20,fproj=0,bflag=TRUE,bmsy=refpts(object)["msy","ssb"]){
  
  if(bflag){
    n1=(n/4)
    mlt=c(seq(1,0.25,length.out=n1)[-n1],seq(0.25,0,length.out=n-n1+1)[-(n-n1+1)])
    btar=FLQuant(mlt*c(bmsy),
...
rebuildTime<-function(object,stk=rebuild(object),bmsy=refpts(object)["msy","ssb"]){
  dt1=transmute(as.data.frame(ssb(stk),drop=T), 
                SSB    =data/c(bmsy),
                Initial=c(ssb(stk[,1]))[an(ac(iter))]/c(bmsy),
                Year   =year)
  
  t=suppressWarnings(as.data.frame(akima:::interp(dt1$Initial,
                                                  dt1$SSB,dt1$Year,
                                                  xo=seq(0,1,length.out=202)[-c(1,202)],
                                                  yo=1,duplicate="mean")))
